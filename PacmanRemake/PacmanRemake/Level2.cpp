...
void Level2::CreatePellet(Vector2D position)
{
    Pellet_Entity = new EntityPellet(m_renderer, "Images/Coin.png", position, m_level_map);
    m_pellets.push_back(Pellet_Entity);
}

void Level2::CreateGhost(string file, Vector2D position)
{
	Ghost_Entity = new EntityGhost(m_renderer, file, m_level_map, position, FACING_RIGHT, LEVEL2_SPEED);
	m_ghost.push_back(Ghost_Entity);
}

void Level2::UpdatePellet(float deltaTime, SDL_Event e)
{
	if (!m_pellets.empty())
	{
...
void Level2::UpdateGhost(float deltaTime, SDL_Event e)
{
...
void Level2::SetLevel2Map()
{
	ifstream infile;
	infile.open("LevelMap/Map2.txt");
...
void Level2::SetUpLevel2()
{
	...
	CreateGhost("Images/GhostAnimation5.png", Vector2D(200, 64));
	CreateGhost("Images/GhostAnimation6.png", Vector2D(200, 512));
